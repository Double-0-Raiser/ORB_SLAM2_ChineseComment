...
// 从pMap中取出关键帧和地图点，并调用BundleAdjustment函数
 ...
 *         + InfoMatrix: invSigma2(与特征点所在的尺度有关)
 *         
 * @param pKF        mpCurrentKeyFrame当前帧
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool *pbStopFlag, Map *pMap)
{
    ...
    // 步骤4：遍历所以有局部MapPoints，得到能被局部MapPoints观测到，但不属于局部关键帧的关键帧，这些关键帧在局部BA优化时不优化
    ...
    if (bDoMore)
    {
        ...
        // 步骤12：对误差较大的outlier设置不优化后，后再次优化
    ...
    // Check inlier observations
    // 步骤13：在优化后重新计算误差，剔除连接误差比较大的关键帧和MapPoint的观测关系
    for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++)
    {
        g2o::EdgeSE3ProjectXYZ *e = vpEdgesMono[i];
        MapPoint *pMP = vpMapPointEdgeMono[i];
