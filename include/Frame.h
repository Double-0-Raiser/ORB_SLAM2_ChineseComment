...
class Frame
{
  ...
  public:
    ...
    float mThDepth;// mbf * (float)fSettings["ThDepth"] / fx 相当于是ThDepth倍基线长度以内为近点
    ...
    // 在双目Frame初始化的时候，通过两个线程分别提取左、右图像的特征点
    std::vector<cv::KeyPoint> mvKeys, mvKeysRight;
    // 如果没有矫正参数，则mvKeysUn就是mvKeys，否则是通过mvKeys提取特征点坐标，利用undistortPoints进行矫正
    ...
    std::vector<float> mvDepth;// 针对双目，就是mbf/disparity
    ...
    static long unsigned int nNextId;// 下一帧Frame的ID
    long unsigned int mnId;// 当前帧Frame的ID，获取nNextId的值之后，nNextId进行自加操作
